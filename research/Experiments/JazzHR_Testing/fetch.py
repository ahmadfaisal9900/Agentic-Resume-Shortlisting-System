...
def fetch_resume(app):
    ...
    app_id = app.get("id")
    display_name = app.get("name") or app.get("first_name", "") + " " + app.get("last_name", "")
    ...
    for attempt in range(1, RETRY_MAX + 1):
        try:
            ...
            if r.status_code == 200:
                ...
                if ctype in ("application/pdf",): ext = ".pdf"
                if ctype in ("application/msword",): ext = ".doc"
                if ctype in ("application/vnd.openxmlformats-officedocument.wordprocessingml.document",): ext = ".docx"
