...
from agentic_hr.utils.env import GEMINI_API_KEY, LANCE_API_KEY, LANCE_REGION, LANCE_URI
...
def node_rerank(state: RAGState) -> dict:
    ...
    idxs, scores = rerank_with_gemini(
        q, snippets, client, EMBEDDINGS_MODEL_ID, top_k=TOPK_RERANK
    )
...
def node_generate(state: RAGState) -> dict:
    ...
    for i, (candidate_id, score) in enumerate(zip(top_ids, top_scores)):
        if i < len(snippets):
            context_lines.append(
                f"Candidate {i + 1} (Score: {score:.3f}): {snippets[i]}"
            )
    context = "\n".join(
        [
    ...
    class AnswerJD(dspy.Signature):
        ...
        answer: str = dspy.OutputField(
            desc="Structured answer explaining top candidates and reasoning"
        )
