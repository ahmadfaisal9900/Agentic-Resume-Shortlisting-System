import logging
from agentic_hr.utils.env import (
    LANCE_URI,
    LANCE_API_KEY,
    LANCE_REGION,
    DOCS_FOLDER,
...
from agentic_hr.components.candidate_ingest import (
    load_meta_docs,
    build_text_rows,
    CAND_TABLE,
    TEXT_TABLE,
...
def main():
    ...
    docs = [
        Document(
            page_content=j["description"], metadata={"id": j["id"], "title": j["title"]}
        )
        for j in jobs
    ]
    ...
    print(
        "Top scores:", [round(s, 4) for s in result_state.get("ranked_scores", [])[:5]]
    )
